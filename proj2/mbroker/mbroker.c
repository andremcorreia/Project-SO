...
static void sig_handler(int signum) {
    if(signum == SIGINT){
        ...
        close(self_pipe);
        cleanPipe(pipe_name);
        printf("Mbroker is shutting down\n");
    }
}
struct mainNode {
    char box_name[32];
...
void addBox(struct linkedList* list, char* box_name) {
    if (pthread_mutex_lock(&boxes_mutex) == -1) {
    ...
    if (pthread_mutex_unlock(&boxes_mutex) == -1) {
...
void removeBox(struct linkedList* list, char* box_name) {
    ...
    while (curr != NULL) {
        if (!strcmp(curr->box_name,box_name)) {
            ...
            if (pthread_mutex_lock(&curr->alarm_lock) == -1) {
                WARN("failed to lock mutex: %s", strerror(errno));
                return;
            }

            curr->remove_flag = false;
            pthread_cond_broadcast(&curr->alarm);

            if (pthread_mutex_unlock(&curr->alarm_lock) == -1) {
                WARN("failed to unlock mutex: %s", strerror(errno));
                return;
            }
...
void publisher(char* clientPipe,char* boxName){
    ...
    if (f == -1){
        printf("Failed to open file in tfs\n");
        box->has_publisher = false;
        close(receiving_pipe);
        return;
    }
    char pub_message[1024];
    ...
    while (!ended && active)
    {    
        ssize_t readBytes = read(receiving_pipe, pub_message, sizeof(pub_message));
        box = findBox(&boxes,boxName);
        if (readBytes > 0 && box != NULL)
        {

            ssize_t written = tfs_write(f, pub_message, strlen(pub_message) + 1);
            box->bytes_written += written;
            if (pthread_mutex_lock(&box->alarm_lock) == -1) {
                WARN("failed to lock mutex: %s", strerror(errno));
                close(receiving_pipe);
                return;
            }
            pthread_cond_broadcast(&box->alarm);
            if (pthread_mutex_unlock(&box->alarm_lock) == -1) {
                WARN("failed to unlock mutex: %s", strerror(errno));
                close(receiving_pipe);
                return;
            }
...
void subscriber(char* clientPipe, char* boxName){
    ...
    while (!ended)
    {
        ...
        if(contents == box->bytes_written && !box->remove_flag){
            free(buf2);
            box->sub_count--;
            close(sub_pipe);
            cleanPipe(clientPipe);
...
void managerReplier(uint8_t code, char* clientPipe, char* error_message){
    ...
    if(send_buffer == NULL) {
        printf("Error: malloc failed in mbroker\n");
        free(send_buffer);
        return;
    }
    ...
    free(send_buffer);
    if (w < 0) {                                                                  
        fprintf(stderr, "[ERR]: write failed in mbroker: %s\n", strerror(errno));
        return;
    }
...
void managerList(char* clientPipe){
    ...
    // alarm to update the threads in case a subscriber disconnected
    if (pthread_mutex_lock(&curr->alarm_lock) == -1) {
        WARN("failed to lock mutex: %s", strerror(errno));
        return;
    }
    curr->remove_flag = true;
    pthread_cond_broadcast(&curr->alarm);
    if (pthread_mutex_unlock(&curr->alarm_lock) == -1) {
        WARN("failed to unlock mutex: %s", strerror(errno));
        return;
...
int main(int argc, char **argv) {
    ...
    if (mkfifo(argv[1], 0666) != 0) {
        fprintf(stderr, "[ERR]: mkfifo failed in mbroker: %s\n", strerror(errno));
        exit(EXIT_FAILURE);
    }

    if(signal(SIGINT, sig_handler) == SIG_ERR){
        printf("error sig in mbroker\n");
        cleanPipe(argv[1]);
        exit(EXIT_FAILURE);
    }
