...
int main(int argc, char **argv) {
    ...
    if (mkfifo(argv[1], 0666) != 0) { // permissions changed to allow read and write for all users
        fprintf(stderr, "[ERR]: publisher mkfifo failed: %s\n", strerror(errno));
    ...
    if(signal(SIGINT, sig_handler) == SIG_ERR) {
        printf("signal error in publisher\n");
        cleanPipe(argv[1]);
    ...
    if(register_buffer == NULL) {
        printf("Error: malloc failed in publisher registration\n");
    ...
    {   
        ...
        if (fgets(message, sizeof(message), stdin) != NULL)
        {
            ...
            if (strlen(message) > 0)
            {
                ...
                if(w == -1){
                    printf("Connection denied by mbroker\n");
                    break;
                }
            }
        }
        else {
            printf("Received EOF or CTRL-C\n");
            break;
        }
    }
